...
try {
  ...
  app.delete("/api/projects/:id", async (req: any, res: any) => {
    try {
      ...
      if (result.rowCount === 0) { res.status(404).json({ error: "プロジェクトが見つかりません" }); return; }
      res.status(204).end();
    } catch (e: any) { res.status(500).json({ error: "サーバーエラー" }); }
  });
  ...
  app.get("/api/projects/:projectId/issues", async (req: any, res: any) => {
    try {
      ...
      let sql = `SELECT i.*, p.project_key, (p.project_key || '-' || i.issue_number) as issue_key
        FROM issues i JOIN projects p ON p.id = i.project_id WHERE i.project_id = $1`;
  ...
  app.get("/api/projects/:projectId/issues/:issueId", async (req: any, res: any) => {
    try {
      const issue = await db.queryOne(`SELECT i.*, p.project_key, (p.project_key || '-' || i.issue_number) as issue_key
        FROM issues i JOIN projects p ON p.id = i.project_id WHERE i.id = $1 AND i.project_id = $2`,
  ...
  app.delete("/api/projects/:projectId/issues/:issueId", async (req: any, res: any) => {
    try {
      const result = await db.run("DELETE FROM issues WHERE id = $1 AND project_id = $2", [req.params.issueId, req.params.projectId]);
      if (result.rowCount === 0) { res.status(404).json({ error: "課題が見つかりません" }); return; }
      res.status(204).end();
    } catch (e: any) { res.status(500).json({ error: "サーバーエラー" }); }
  });
  ...
  app.patch("/api/projects/:projectId/issues/:issueId/status", async (req: any, res: any) => {
    try {
      ...
      const issue = await db.queryOne(`
        UPDATE issues SET status_id = $1, updated_at = NOW() WHERE id = $2 AND project_id = $3
  ...
  app.get("/api/issues/:issueId/comments", async (req: any, res: any) => {
    try {
      const comments = await db.query(`SELECT c.*, u.name as user_name, u.avatar_url as user_avatar_url
        FROM comments c JOIN users u ON u.id = c.user_id WHERE c.issue_id = $1 ORDER BY c.created_at DESC`, [req.params.issueId]);
      res.json(comments);
    } catch (e: any) { res.status(500).json({ error: "サーバーエラー" }); }
  });
  ...
  app.post("/api/issues/:issueId/comments", async (req: any, res: any) => {
    try {
      ...
      if (!issue) { res.status(404).json({ error: "課題が見つかりません" }); return; }
      const comment = await db.queryOne(`INSERT INTO comments (issue_id, user_id, content) VALUES ($1, $2, $3)
  ...
  app.delete("/api/comments/:commentId", async (req: any, res: any) => {
    try {
      ...
      if (result.rowCount === 0) { res.status(404).json({ error: "コメントが見つかりません" }); return; }
      res.status(204).end();
    } catch (e: any) { res.status(500).json({ error: "サーバーエラー" }); }
  });

  // アバター（Supabase Storage）
  ...
  app.post("/api/avatars/upload", upload.single("avatar"), async (req: any, res: any) => {
    try {
      ...
      const { error } = await supabaseClient.storage.from("avatars").upload(filename, req.file.buffer, { contentType: req.file.mimetype, upsert: true });
...
export default function handler(req: any, res: any) {
  if (initError) {
    res.status(500).json({ error: initError.message, stack: initError.stack?.split("\n").slice(0, 5) });
    return;
  }
  app(req, res);
}
